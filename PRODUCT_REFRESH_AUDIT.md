...
6. **Secondary-flow mobile pass:** kept plan creation/saving available as a focused mobile action, brought the mock-exam start command above the bottom navigation, and reduced initial Word Library and Sentence Library batches while preserving full search and incremental loading.
7. **Focused session and completion pass:** removed global mobile navigation from active learning sessions so it cannot cover feedback or continuation controls, compacted mobile Grammar result metrics, completed the placement/reader/grammar feedback and result audit, and verified saved mock-exam answers, flags, timing, resume, submission, and History persistence.
